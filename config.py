...
LLM_PROVIDER_ROUTER = os.getenv("LLM_PROVIDER_ROUTER", LLM_PROVIDER)
MODEL_NAME_ROUTER = os.getenv("MODEL_NAME_ROUTER", MODEL_NAME)

## Summerize LLM 설정
LLM_PROVIDER_GENERATOR = os.getenv("LLM_PROVIDER_GENERATOR", LLM_PROVIDER)
MODEL_NAME_GENERATOR = os.getenv("MODEL_NAME_GENERATOR", MODEL_NAME)

# Hollucination Check LLM 설정
LLM_PROVIDER_GRADER = os.getenv("LLM_PROVIDER_GRADER", LLM_PROVIDER)
MODEL_NAME_GRADER = os.getenv("MODEL_NAME_GRADER", MODEL_NAME)

# Rewite query LLM 설정
LLM_PROVIDER_REWRITER = os.getenv("LLM_PROVIDER_REWRITER", LLM_PROVIDER)
MODEL_NAME_REWRITER = os.getenv("MODEL_NAME_REWRITER", MODEL_NAME)
...
ROUTER_PROMPT = """You are an expert at routing a url to a fetch_html_tool, coopang_html_tool and vision_model_parser.
fetch_html_tool : any URL that is NOT from coupang.com and is NOT an image.
coopang_html_tool : URLs hosted on coupang.com.
vision_model_parser : nothing
"""
...
- product_name (문자열): 실제 상품명. 입력 텍스트에서 가장 먼저 등장하는 정식 상품명을 그대로 사용합니다. 불필요한 단위나 설명은 제거합니다.
- total_price (정수): 최종 판매가(할인 적용 후). ‘원’ 단위를 제거하고 숫자만 추출합니다.
- count (정수): 제목에서 상품 개수. “2개”, “'500g x 2팩'에서 2팩” 등 표기된 수량을 파싱합니다. 개수 단위가 있는지 반드시 확인하고 파싱합니다, 수량 표기가 없으면 1로 간주합니다. 단위는 제거합니다.
